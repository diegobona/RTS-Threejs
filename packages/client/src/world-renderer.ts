...
export class WorldRenderer {
  ...
  private rebuildBuildings(): void {
    ...
    for (const e of this.world.entities.values()) {
      ...
      if (!real && !art) continue;
      const sp = new Sprite(real ? real.tex : art!.tex);
      ...
      } else if (art!.anchorMode === 'footprintCenter') {
        sp.anchor.set(art!.spriteAnchorX ?? 0.5, art!.spriteAnchorY ?? 0.85);
        sp.scale.set(art!.scale ?? 1);
        const cx = (cornerX(e.cellX, e.cellY) + cornerX(e.cellX + fw, e.cellY + fh)) / 2;
        const cy = (cornerY(e.cellX, e.cellY) + cornerY(e.cellX + fw, e.cellY + fh)) / 2;
        sp.position.set(cx, cy + TILE_H / 2);
        if (art!.tint !== false) sp.tint = this.playerColor(e.owner);
      } else {
  ...
  render(alpha: number, selected: ReadonlySet<number>): void {
    ...
    for (const e of this.world.entities.values()) {
      ...
      if (!v) {
        ...
        const custom = real ? null : this.art.unitTextures.get(e.typeId);
        const body = new Sprite(
          real ? real.tex : custom ? custom.tex : type.domain === 'vehicle' ? this.art.vehicleBody : this.art.infantryBody,
        );
        if (real) {
          body.anchor.set(real.anchorX / body.texture.width, real.anchorY / body.texture.height);
        } else if (custom) {
          body.anchor.set(custom.spriteAnchorX ?? 0.5, custom.spriteAnchorY ?? 0.9);
          body.scale.set(custom.scale ?? 1);
          if (custom.tint !== false) body.tint = this.playerColor(e.owner);
        } else {
          body.anchor.set(0.5);
        ...
        // 真实体素载具自带炮塔，无需占位炮管
        const barrel =
          !realVeh && !custom && type.weapon && type.domain === 'vehicle' ? new Sprite(this.art.vehicleBarrel) : null;
        if (barrel) {
      ...
      v.body.zIndex = sy;
      v.body.scale.set(v.baseScale * (selected.has(e.id) ? 1.15 : 1));
      // 敌方单位仅在可见格显示（迷雾隐藏）
