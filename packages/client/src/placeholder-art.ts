...
export interface ArtAssets {
  ...
  buildingTextures: Map<string, BuildingTexture>;
  unitTextures: Map<string, UnitTexture>;
...
export interface BuildingTexture {
  ...
  tint?: boolean;
  scale?: number;
  anchorMode?: 'topLeft' | 'footprintCenter';
...
export async function loadWw1BuildingArt(): Promise<Map<string, BuildingTexture>> {
  const buildingArt = new Map<string, BuildingTexture>();
  for (const spec of WW1_BUILDING_ART) {
    buildingArt.set(spec.typeId, {
      tex: await Assets.load<Texture>(spec.src),
      anchorX: 0,
      anchorY: 0,
      tint: false,
      scale: spec.scale,
      anchorMode: 'footprintCenter',
      spriteAnchorX: spec.spriteAnchorX,
      spriteAnchorY: spec.spriteAnchorY,
    });
  }
  return buildingArt;
}

export async function loadWw1UnitArt(): Promise<Map<string, UnitTexture>> {
  const unitArt = new Map<string, UnitTexture>();
  for (const spec of WW1_UNIT_ART) {
    unitArt.set(spec.typeId, {
      tex: await Assets.load<Texture>(spec.src),
      tint: false,
      scale: spec.scale,
      spriteAnchorX: spec.spriteAnchorX,
...
export function buildArt(
  ...
  customBuildings: ReadonlyMap<string, BuildingTexture> = new Map(),
  customUnits: ReadonlyMap<string, UnitTexture> = new Map(),
): ArtAssets {
  const buildingTextures = new Map<string, BuildingTexture>();
  const unitTextures = new Map<string, UnitTexture>(customUnits);

  for (const u of units) {
    if (u.domain !== 'building' || !u.building) continue;
    const custom = customBuildings.get(u.id);
    if (custom) {
      buildingTextures.set(u.id, custom);
      continue;
    }
  ...
  return { buildingTextures, unitTextures, infantryBody, vehicleBody, vehicleBarrel };
}

/** 生成单位/建筑的 cameo 图标（侧边栏建造按钮用）。 */
export function makeCameo(typeId: string, name: string): HTMLElement {
  const src = ww1ArtSrc(typeId);
  if (src) {
    const wrap = document.createElement('div');
    wrap.style.cssText =
      'width:60px;height:48px;background:linear-gradient(#5d6973,#182027);display:flex;align-items:center;justify-content:center;';
